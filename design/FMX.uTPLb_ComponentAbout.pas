...
unit FMX.uTPLb_ComponentAbout;

interface

//** Converted with Mida BASIC 280     http://www.midaconverter.com
...
uses
  ...
  Fmx.StdCtrls,
  FMX.Header,
  FMX.Graphics, FMX.WebBrowser, FMX.ScrollBox, FMX.Controls.Presentation;
...
type
  TTPLb_fmComponentAbout = class(TForm)
    ...
    lblTpsfaIntro: TLabel;
    lblTpsfaContact: TLabel;
    lblWebLabel: TLabel;
    imgSean: TImage;
...
uses
  Winapi.Windows, FMX.Platform.Win, FMX.uTPLb_InfoUtils, uTPLb_Hash, uTPLb_HashDsc,
  uTPLb_StreamCipher, uTPLb_Signatory, uTPLb_Codec, TypInfo, uTPLb_BlockCipher,
  Registry, uTPLb_I18n, uTPLb_Decorators;
...
case Idx of
  -1: begin
      imgAuthorPic.Bitmap := nil;
      sAuthorEmail := '';
      sAuthorWeb   := ''
      end;
   0: begin
      imgAuthorPic.Bitmap := imgSean.Bitmap;
...
if sAuthorEmail <> '' then
    lblAuthorEmail.Text := DS_Email + sAuthorEmail
  else
    lblAuthorEmail.Text := '';
if sAuthorWeb <> '' then
    lblAuthorWeb.Text := DS_Web + sAuthorWeb
  else
    lblAuthorWeb.Text := '';
end;
...
if (Sender is TLabel) then
  begin
  s := TLabel( Sender).Text;
  ShellExecute( WindowHandleToPlatform(Handle).Wnd, 'open', PChar( s), nil, nil, SW_SHOWNORMAL)
  end
end;
...
lblRunTimeVersion.Text := Format( DS_RunTimeIs,
  [LibName, FileVersion]);
Inc( ErrorPos);
Get_dclTP_LockBox3_Info( LibName, FileVersion);
lblDesignTimeVersion.Text := Format( DS_DesignTimeIs,
  [LibName, FileVersion]);
Inc( ErrorPos);
...
if assigned( HashComp) then
    begin
    lblHashDisplayName.Text   := HashComp.Hash;
    PopulateListBoxWithFeatures( HashComp.Features, lbxHashFeatures)
    end
  else
    lblHashDisplayName.Text   := DS_HashNotSelected;

Inc( ErrorPos);
if assigned( HashDsc) then
    begin
    lblHashDefinitionURL.Text := HashDsc.DefinitionURL;
    lblHashWikipediaURL.Text  := WikipediaRelativeURL( HashDsc.WikipediaReference);
    lblHashBlockSize.Text     := Format( DS_BlockSizeEqs, [HashDsc.UpdateSize]);
    lblHashDigestSize.Text    := Format( DS_DigestSizeEqs, [HashDsc.DigestSize]);
    end
  else
    begin
    lblHashDefinitionURL.Text := '';
    lblHashWikipediaURL.Text  := '';
    lblHashBlockSize.Text     := '';
    lblHashDigestSize.Text    := '';
    end;
...
if assigned( Chain) then
    begin
    lblChainName.Text := Chain.DisplayName;
    PopulateListBoxWithFeatures( Chain.Features, lbxChainFeatures);
    lblChainWikipedia.Text  := WikipediaRelativeURL( Chain.WikipediaReference)
    end
  else
    begin
    lblChainName.Text := DS_ChainModeNotSelected;
    lblChainWikipedia.Text  := ''
    end;


Inc( ErrorPos);
if assigned( CodecComp) then
    lblCipherDisplayName.Text := CodecComp.Cipher
  else
    lblCipherDisplayName.Text := DS_CodecNotSelected;
...
if assigned( StreamCipher) then
    ...
    lblCipherDefinitionURL.Text := StreamCipher.DefinitionURL;
    lblCipherWikipediaURL.Text  := WikipediaRelativeURL( StreamCipher.WikipediaReference);
    lblCipherBlockSize.Text := ''
    end
    ...
    lblCipherDefinitionURL.Text := BlockCipher.DefinitionURL;
    lblCipherWikipediaURL.Text  := WikipediaRelativeURL( BlockCipher.WikipediaReference);
    lblCipherBlockSize.Text := Format( DS_BlockSizeEqs, [BlockCipher.BlockSize])
    end
  else
    begin
    ErrorPos := 40;
    lblCipherDefinitionURL.Text := '';
    lblCipherWikipediaURL.Text  := '';
    lblCipherBlockSize.Text := ''
    end
